...
struct BlockSize {
  ...
  static constexpr std::uint64_t byteOffsetToBlock(std::uint64_t byteOffset) {
    return byteOffset >> size_log2;
  }

  static constexpr std::uint64_t blockToByteOffset(std::uint64_t block) {
    return block << size_log2;
  }

  static constexpr std::uint64_t alignByteOffset(std::uint64_t byteOffset) {
