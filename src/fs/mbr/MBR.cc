...
fs::Directory::ptr_type fs::mbr::tryMakeDirectory(const ::fs::File::ptr_type& device) {
  ...
  for (auto begin = mbr.partitionTable().begin(), it = begin, end = mbr.partitionTable().end(); it != end; ++it) {
    if (!it->isExFat()) {
      continue;
    }

    auto partition = FileSlice::tryMake(device, it->offset(device), it->size(device));
    if (partition == nullptr) {
      continue;
    }

    char index = static_cast<char>('0' + (it - begin));
    directory->put(Directory::name_type(1, index), std::move(partition));
  }
