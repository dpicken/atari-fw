...
void platform::rp2040::App::run(::keyboard::App& app) {
  m_keyboardApp = &app;
  for(;;) {
    tuh_task();
...
void platform::rp2040::App::run(::sio::App& app) {
  m_sioApp = &app;
  for (;;) {
    m_sioApp->schedule();
  }
